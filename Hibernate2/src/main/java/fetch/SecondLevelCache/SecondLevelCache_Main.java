...
//		tx.commit();
		...
		// Caching using HQL :
		Query q1 = session1.createQuery("from School where scid = 3");		// "School_Details" is the EntityName ; Also ByDefault EntityName is the Class name, untill we specifically change it by doing  @Entity(name = "School_Details") in School class.  & we can use instance variable name "scid" or entityColumnName "school_id"
																			// But when we only change  @Table(name = "School_Details") ,  and no change in @Entity name, then in that case again our class name will be Entity Name, that we have to put here in HQL query 
...
/*		// Second level Caching ::
		...
		// Caching using HQL  :
		Query q2 = session2.createQuery("from School where scid = 3");
